...
public class DaoProducto implements Dao<Producto>{
    ...
    public Producto select(int id) throws SQLException {
        ...
        try(PreparedStatement ps = con.prepareStatement("SELECT * FROM tienda.productos WHERE codigo = ?")){
...
//	        ps.close();
    ...
    public void update(Producto p) throws SQLException{
        try(PreparedStatement ps = con.prepareStatement("UPDATE tienda.producto SET nombre = ? "
        		+ "WHERE codigo = ?")){
        	ps.setString(1, p.getNombre());
